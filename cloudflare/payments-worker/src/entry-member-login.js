...
async function createSetupCheckout(env, memberSession, summary) {
  ...
  return stripeRequest(env, "POST", "checkout/sessions", {
    ...
    locale: "es",
    success_url: `${base}/portal.html?token=${token}&billing=payment-updated&setup_session_id={CHECKOUT_SESSION_ID}`,
    cancel_url: `${base}/portal.html?token=${token}&billing=payment-canceled`,
...
async function createResubscribeCheckout(env, memberSession, summary) {
  ...
  return stripeRequest(env, "POST", "checkout/sessions", {
    ...
    locale: "es",
    success_url: `${base}/portal.html?token=${token}&billing=resubscribed`,
    cancel_url: `${base}/portal.html?token=${token}&billing=resubscribe-canceled`,
    "metadata[align_group_id]": memberSession.groupId,
