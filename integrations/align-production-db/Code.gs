...
function sheets_() {
  ...
  return {
    ...
    socios: sheet_(ss, 'SOCIOS', [
      'Socio ID','Usuario','Titular','Plan','Estado','Alta','Integrantes','Pagos acumulados','Gasto aliados','Valor total','Visitas','Personas','Última visita','Notas','Fuente','Password hash','Stripe Customer','Stripe Subscription','Último pago','Estado Stripe'
    ]),
...
function registerPayment_(body) {
  ...
  if (!socioRow) {
    ...
    db.socios.getRange(socioRow, 1, 1, 20).setValues([[
      socioId,
      username,
